...
<title>Admin Dashboard &mdash; Car Shop Management</title>
...
<main class="main-content">
    <div class="page-header">
        <div>
            <h1 class="page-title">Manage Salespeople</h1>
            <p class="page-sub">Register, edit, search and remove salesperson accounts</p>
        </div>
    </div>

    <?php if (isset($_GET['msg'])): ?>
        <?php $messages = ['added' => 'Salesperson added successfully.',
    ...
    <div class="card form-card">
        <h3 class="card-title">
            <?= $isEdit ? '&#9998; Edit Salesperson (#' . intval($editing['id']) . ')' : '+ Add New Salesperson' ?>
        </h3>
        <form method="POST"
            ...
            <div class="field-row">
                <div class="field">
                    <label for="name">Salesperson Name</label>
    ...
    <div class="card">
        <div class="card-toolbar">
            ...
            <span class="badge" id="resultCount"><?= count($salespeople) ?> total</span>
        </div>
        ...
        <div class="table-wrap">
            <table class="data-table">
                ...
                <tbody id="tableBody">
                    <?php if (empty($salespeople)): ?>
                        <tr><td colspan="5" class="empty">No salespeople yet.</td></tr>
                    <?php else: ?>
                        <?php foreach ($salespeople as $i => $salesperson): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td><?= htmlspecialchars($salesperson['name']) ?></td>
                                <td><?= htmlspecialchars($salesperson['contact']) ?></td>
                                <td><?= htmlspecialchars($salesperson['username']) ?></td>
                                <td class="text-right">
                                    <a class="btn-sm btn-edit"
                                       href="index.php?page=admin&action=edit&id=<?= $salesperson['id'] ?>">Edit</a>
                                    <a class="btn-sm btn-delete"
                                       href="index.php?page=admin&action=delete&id=<?= $salesperson['id'] ?>"
                                       onclick="return confirm('Delete this salesperson?')">Delete</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
...
<footer class="footer">&copy; <?= date('Y') ?> Car Shop Management System</footer>
...
(function () {
    ...
    input.addEventListener('input', function () {
        ...
        timer = setTimeout(function () {
            fetch('index.php?page=ajax&type=salesperson&q=' + encodeURIComponent(input.value.trim()),
